...
(function() {
  ...
  _.each = function(collection, iterator) {
    if (collection.length) { // Checks if 'collection' is an array, b/c an object would return 'undefined' here.
      for (var i=0; i<collection.length; i++) {
        iterator(collection[i], i, collection);
      }
    } else { // If 'collection' isn't an array, treats it as an object.
  ...
  _.filter = function(collection, test) {
    var result = [];
    // Uses _.each() from above rather than writing a new for loop.
  ...
  _.invoke = function(collection, functionOrKey, args) {
    if (typeof functionOrKey !== "function") {
      functionOrKey = new Function("return this."+functionOrKey+"();");
    }
    // _.map() already returns an array of results from a given parameter.
  ...
  _.shuffle = function(array) {
    ...
    var shuffled = [];
    while (args.length > 0) {
      // pointer generates a random number from the available indices of args.
      var pointer = Math.floor(Math.random() * args.length);
      shuffled.push(args.splice(pointer, 1)[0]);
    }
    return shuffled;
  };
